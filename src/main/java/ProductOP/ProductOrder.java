...
public class ProductOrder {
    ...
    public void duplicateNodesOp(){
        ...
        for (ProductNode node : nodes) {
            ...
            ArrayList<ProductNode> duplicateNodes = productTree.getDuplicateNodes(node);

            if (duplicateNodes.size() > 1) {

                for (ProductNode duplicateNode : duplicateNodes) {
                    duplicateNodes.get(0).getProductionLists().setGrossRequirement(addGrossRequirements(duplicateNodes.get(0), duplicateNode));
                }
    ...
    public ArrayList<Integer> operationsOfNetRequirement (ArrayList<Integer> gross, ArrayList<Integer> prior,ArrayList<Integer> scheduled){
        ...
        ArrayList<Integer> list =new ArrayList<>();
        for(int i=0; i<10; i++){
            int number = Math.max((gross.get(i) - (prior.get(i) + scheduled.get(i))), 0);
            list.add(number);
        }
        return list;
    }
    ...
    public ArrayList<Integer> operationsTimePhasedNetRequirement(ProductNode node, ArrayList<Integer> netRequiremnt){
        ...
        for(int i=0; i<10; i++){
            if(netRequiremnt.get(i)!=0 &&(i-leadTime)<0){
                JOptionPane.showMessageDialog(null,"We can't produce what you want in this time period");
                throw new ArithmeticException("We can't produce what you want in this time period");
            }
